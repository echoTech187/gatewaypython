    ...
    def run(self):
        ...
        def callback(ch, method, properties, body):
            ...
            if "msgType" in data and "msgInfo" in data:
                if data['msgType'] == "consumer_waiting_success_dynamic_qris_mpm":
                    payload = json.dumps({
                    ...
                    try:
                        ...
                        response = requests.request("POST", internal_url_hit+"/WaitingSuccess/dynamicQrisMpm", headers=headers, data=payload, timeout=30)
...
queue_name = "queue_waiting_success_dynamic_qris_mpm"
max_retries = 3
