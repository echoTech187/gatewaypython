    ...
    def run(self):
        ...
        def callback(ch, method, properties, body):
            ...
            if "msgType" in data and "msgInfo" in data:
                if data['msgType'] == "consumer_waiting_success_transfer_bifast":
                    payload = json.dumps({
                    ...
                    try:
                        ...
                        response = requests.request("POST", internal_url_hit+"/WaitingSuccess/transferBifast", headers=headers, data=payload, timeout=30)
...
queue_name = "queue_waiting_success_transfer_bifast"
max_retries = 3
